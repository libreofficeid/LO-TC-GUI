...
def elevate(cmd, fname, show_console=True, graphical=True):
    ...
    params = ShellExecuteInfo(
        ...
        hwnd=None,
        lpVerb=b'runas',
        lpFile="{}".format(sys.executable+"\\..\\python").encode('cp1252'),
        lpParameters="{}".format(TMPDIR+"\\\\"+fname).encode('cp1252'),
...
def elevate_commands(cmd, fname):
    import os
    try:
        ret = elevate(cmd, fname)
        if ret == 0:
            if os.path.exists(TMPDIR+"\\\\"+fname):
